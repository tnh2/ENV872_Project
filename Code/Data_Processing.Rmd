...
```{r Geospatial}
#county_bounds_pop <- left_join(county_bounds, pop_processed, by = c("GEOID" = "FIPS"))
#coal_counties <- pop_processed %>%
#  filter(FIPS %in% as.list(mines$mctyfips))
...
coal_counties2 <- filter(county_bounds, COUNTYFP %in% unique(mines$mctyfips))

diff <- filter(coal_counties_tab, COUNTYFP %notin% unique(coal_counties2$COUNTYFP))

cont.mines <- filter(mines, state != "Alaska" & state != "Hawaii")
cont.states <- state_bounds_WGS %>%
  filter(STATEFP %in% c(1, 3:14, 16:56)) %>%
  droplevels()
cont.counties <- filter(county_bounds_WGS, STATEFP %in% c(1,3:14,16:56))
## Visual Validation of intersection command
ggplot() +
  #geom_sf(data = filter(state_bounds_WGS, STATEFP == 42)) +
  #geom_sf(data = filter(county_bounds_WGS, STATEFP == 42), fill = "white") +
  geom_sf(data = filter(coal_counties_spatial, STATEFP == 42)) +
  #geom_sf(data = filter(cont.mines, mstafips == 42))
...
covid_all_processed <- covid_all_raw %>%
  ...
  na.exclude(FIPS)
covid_all_processed$FIPS <- sprintf("%05i", covid_all_processed$FIPS)
...
WV.Small.Mines <- filter(WV.Small, Mines.Label == "Mines")
WV.Small.NoMines <- filter(WV.Small, Mines.Label == "No Mines")
