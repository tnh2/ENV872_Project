...
covid_all_raw <- read.csv(here("Data", "Raw", "COVID_County_04-18-2020.csv"))
...
#pop_processed$FIPS <- sprintf("%05d", pop_processed$FIPS)
...
# KY_avg <- pop_processed %>%
#   filter(STNAME == "Kentucky") %>%
#   mutate(Above_avg = case_rate > coal_state_cases$case_rate[coal_state_cases$STNAME == STNAME])
# 
# coal_KY_avg <- filter(KY_avg, FIPS %in% coal_counties_tab$GEOID)
